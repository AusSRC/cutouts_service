...
class FitsObjStore:
    ...
    def __init__(self, mode="s3", readsize=ONE_G_9):
        ...
        self.__last_byte_pos = (
            -1
        )  # Position of the last byte that was read (byte numbers start at 0)
        self.__end_header = 0
        self.__stride_len = 0
        self.DEBUG = False
    ...
    def readData(self, start, length):
        ...
        if self.mode == "s3":  # Use Boto3 library
            ...
            obj_content = self.client.get_object(
                Bucket=self.bucket, Key=self.obj, Range=ranges
            )["Body"].read()
    ...
    def getObjectHeaders(self, filtered="FITS"):
        response = self.client.head_object(Bucket=self.bucket, Key=self.obj)[
            "ResponseMetadata"
        ]["HTTPHeaders"]
        flatdict = {}
        if filtered == "FITS":
    ...
    def readWholeObject(self):
        ...
        obj_content = None
        if self.mode == "s3":  # Use Boto3 library
            obj_content = self.client.get_object(Bucket=self.bucket, Key=self.obj)[
                "Body"
            ].read()
        else:
            if length > ONE_G_9:
    ...
    def getChannelBatches(self, xmin, xmax, ymin, ymax, zmin, zmax, ch_nums, i):
        ...
        for j in range(ch_nums - 1):
            ...
            data = np.concatenate(
                (
                    data,
                    self.__extractDataFromChannel(
                        chdata[start:end], xmin, xmax, ymin, ymax
                    ),
                ),
                axis=0,
            )
        ...
        print(
            f"Read {i + 1}: ch {start_ch} = {self.chsize * FITS_FLOAT_SIZE} bytes per ch, {ch_nums} channels started at byte {startpos}",
            flush=True,
        )
        return data
    ...
    def getPartitionDataByStrategy(
        self, xmin, xmax, ymin, ymax, zmin, zmax, hdr, strategy=2
    ):
        ...
        if strategy == 1:
            ...
            for i in range(self.zlen - 1):
                data = np.concatenate(
                    (data, self.getWholeChannel(xmin, xmax, ymin, ymax, zmin, zmax, i)),
                    axis=0,
                )
                if i % 10 == 0:
            ...
            for i in range(self.zlen - 1):
                data = np.concatenate(
                    (
                        data,
                        self.getChannelByRow(xmin, xmax, ymin, ymax, zmin, zmax, i + 1),
                    ),
                    axis=0,
                )
                if i % 10 == 0:
            ...
            for i in range(num_reads - 1):
                data = np.concatenate(
                    (
                        data,
                        self.getChannelBatches(
                            xmin, xmax, ymin, ymax, zmin, zmax, batch, i + 1
                        ),
                    ),
                    axis=0,
                )
            ...
            if extra_batch > 0:
                data = np.concatenate(
                    (
                        data,
                        self.getChannelBatches(
                            xmin, xmax, ymin, ymax, zmin, zmax, extra_batch, i + 1
                        ),
                    ),
                    axis=0,
                )
    ...
    def getPartitionData(self, xmin, xmax, ymin, ymax, zmin, zmax, hdr, num_threads=1):
        ...
        for task in tasks:
            r = pool.apply_async(
                self.getChannelBatches,
                (xmin, xmax, ymin, ymax, zmin, zmax, task[0], task[1]),
            )
