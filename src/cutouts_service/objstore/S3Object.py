    ...
    def uploadFile(
        ...
        if progress:
            self.client.upload_file(
                ...
                ExtraArgs=ExtraArgs,
                Config=config,
                Callback=ProgressPercentage(myfile),
            )
        else:
            self.client.upload_file(
                myfile, self.bucket, self.obj, ExtraArgs=ExtraArgs, Config=config
            )
...
if __name__ == "__main__":
    ...
    (ja3_access_id, ja3_secret_id, quota) = get_access_keys(
        "my_certs.json", endpoint, project
    )
    ...
    if LOADFILE:
        ...
        print(
            "Retrieve %s x %s x %s subcube"
            % (xmax - xmin + 1, ymax - ymin + 1, zmax - zmin + 1)
        )
        fdata = obj.getPartitionData(
            xmin, xmax, ymin, ymax, zmin, zmax, hdr, NUM_THREADS
        )
